...
**"Patient", not "client" — nine times out of ten.** Separate from the US/UK pass above, and applied on UK articles too. Walk every instance of "client" in the article and rewrite it to the patient form: *client* → *patient*, *clients* → *patients*, *clientele* / *client base* → *patient base*, *client record* / *client file* → *patient record* / *patient file*, *client communication* → *patient communication*, *client experience* → *patient experience*. Apply this automatically — never ask about it. Four exceptions, and only these four: (1) **Pabau's own feature names stay verbatim** — the Client Portal, the client card, Client records; rename the people around the feature, never the feature ("With the Client Portal, patients rebook themselves"); (2) text inside a **direct quote or a cited source**; (3) a **competitor's or third party's product name**; (4) a **genuinely non-clinical business or B2B relationship** — a salon, a barber, an agency and its accounts. Full rule in `Pabau-style-guide.md` → "Patient, not client".
