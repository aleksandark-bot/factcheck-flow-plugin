...
- **Image captions** — any image block with no `<figcaption>` at all, or a caption that
  isn't a full sentence ending in a period, isn't wrapped in `<em>`, or carries stray `*`
  asterisks instead of real italic markup. Also flag a caption on a **Pabau feature
  screenshot** that doesn't say how that feature helps the reader do what this article is
  about (a bare feature label or generic product praise is a finding). Report the image by
  its alt text or filename so the fix lands on the right one; the editorial pass writes
  the captions.
